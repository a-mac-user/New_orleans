...
class UserProfileAdmin(UserAdmin):
    ...
    fieldsets = (
        ...
        ('用户权限', {'fields': ('is_active', 'is_staff', 'is_admin', 'roles',
...
class CustomerAdmin(admin.ModelAdmin):
    list_display = ('qq', 'name', 'source', 'phone', 'consult_course',
                    'consultant', 'status', 'date')
    choice_fields = ('status', 'source', 'class_type')
    fk_fields = ('consultant', 'course')
    list_per_page = 10
    list_filter = ('name', 'source', 'consult_course', 'status', 'date', 'consultant')
    list_editable = ['phone', ]
...
class EnrollmentAdmin(admin.ModelAdmin):
    list_display = ('customer', 'course_class', 'school', 'enrolled_date',
                    'contract_agreed', 'contract_approved')


class PaymentRecordAdmin(admin.ModelAdmin):
    list_display = ('enrollment', 'pay_type', 'amount', 'date', 'consultant')


class StudyRecordAdmin(admin.ModelAdmin):
    list_display = ('id', 'student', 'course_record', 'attendance', 'score', 'date', 'memo')
    list_editable = ('student', 'score', 'attendance', 'memo')
