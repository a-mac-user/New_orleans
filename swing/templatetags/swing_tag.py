        ...
        ele = '''
            <div class="searchbox">
               <form method="get" class="span5">
                    ...
                    %s
                    <button class="btn btn-danger" style="margin:0 0 10px 25px">搜索</button>
               </form>
           </div>
...
def render_page_num(request, paginator_obj, loop_counter):
    ...
    if abs(loop_counter - paginator_obj.number) < 2 or loop_counter == 1 or loop_counter ==\
            paginator_obj.paginator.num_pages:  # the first page or last
...
def paginator(obj, data_type):
    ...
    for p_num in obj.paginator.page_range:
        if p_num == obj.number:
            html += '''<li class="active"><a href="?page=%s&type=%s">%s</a></li>''' % (p_num,
                                                                                       data_type,
                                                                                       p_num)
        else:
...
@register.simple_tag
def paginator2(obj, arg_name, arg_val):
    html = '''<ul class="pagination">'''
    if obj.has_previous():
    ...
    for p_num in obj.paginator.page_range:
        if p_num == obj.number:
            html += '''<li class="active"><a href="?page=%s&%s=%s">%s</a></li>''' % (p_num,
                                                                                     arg_name,
                                                                                     arg_val,
                                                                                     p_num)
        else:
...
def get_m2m_obj(rel_field_name, form_obj):
    ...
    try:
        m2m_obj = getattr(form_obj.instance, rel_field_name)
        # print("m2m obj:",m2m_obj)
        return m2m_obj.model.objects.all()
        ...
        # return
        # to deal ValueError: "<UserProfile: >" needs to have a value for field "userprofile" before this
...
@register.simple_tag
def get_chosen_m2m_obj(form_field_obj, model_obj):
    # return chosen m2m obj
    selected_pks = form_field_obj.value()
    try:
        m2m_obj = getattr(model_obj, form_field_obj.name)
        selected_obj = m2m_obj.select_related().filter(id__in=selected_pks)
        # print("get_chosen_m2m_obj", form_field_obj.value(), selected_obj)
        # print(selected_obj.values())
        return selected_obj
...
def recursive_related_obj_lookup(obj, model_name):
    model_name = obj[0]._meta.model_name
    ul_ele = "<ul>"
    for obj in obj:
        # li_ele = '''<li>
        ...
        for related_obj in obj._meta.related_objects:
            ...
            if hasattr(obj, related_obj.get_accessor_name()):
                ...
                if hasattr(accessor_obj, 'select_related'):
                    target_obj = accessor_obj.select_related()  # .filter(**filter_coditions)
                ...
                if len(target_obj) > 0:
                    ...
                    nodes = recursive_related_obj_lookup(target_obj, model_name)
                    ul_ele += nodes
    ul_ele += "</ul>"
    return ul_ele
...
def display_obj_related(obj):
    # 把对象及所有相关联的数据取出来
    if obj:
        # model_class = obj[0]._meta.model
        mode_name = obj[0]._meta.model_name
        return mark_safe(recursive_related_obj_lookup(obj, mode_name))
...
def get_m2m_obj(rel_field_name, form_obj):
    ...
    try:
        ...
        # return model_obj._meta.model.bind_hosts.through.bindhosts.get_queryset()
        # return
        #  to deal ValueError: "<UserProfile: >" needs to have a value for field "userprofile"
...
def get_chosen_m2m_obj(form_field_obj, model_obj):
    ...
    try:
        m2m_obj = getattr(model_obj, form_field_obj.name)
        selected_obj = m2m_obj.select_related().filter(id__in=selected_pks)
        # print("get_chosen_m2m_obj", form_field_obj.value(), selected_objs)
        # print(selected_obj.values())
        return selected_obj
    except Exception as e:
        return []
